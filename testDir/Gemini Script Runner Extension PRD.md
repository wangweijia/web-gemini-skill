...
## **7. 方案评估与优化建议（2026-07-09 追加）**

### **7.1 核心安全隐患：`run_code` 缺乏实质隔离**
* **问题**：原生 `vm` 模块非安全沙箱，AI 生成的代码或恶意注入可通过原型链轻松逃逸，控制宿主机。
* **优化**：建议彻底废弃 `run_code`，全面转由 `run_skill` 承载；或在 CLI 中引入 `isolated-vm` 替代原生 `vm`。

### **7.2 路径握手失败导致的 `alert` 死锁风险**
* **问题**：`content.js` 在握手失败时弹出 `alert()`，而 `onclose` 的 5 秒无条件自动重连会导致弹窗无限循环，卡死网页。
* **优化**：在握手失败等配置类错误触发时，设置状态锁切断自动重连，改为侧边栏红灯提示并等待手动触发。

### **7.3 缺少格式解析失败的 AI 自我修正反馈机制**
* **问题**：若 AI 输出错误的 JSON 格式，插件在 `catch` 捕获后仅在本地切为 error，AI 无法感知，自动流直接中断。
* **优化**：在解析失败时，向网页回填报错文本（如`【GLAB 异常反馈】JSON 解析失败，请检查语法`），给 AI 自动订正的机会。

### **7.4 大文件读取缺乏分页分片支持**
* **问题**：`read_file` 超过 `maxBytes` 自动截断，但协议中没有 `offset` 或 `page` 参数，AI 无法读取大文件的后续上下文。
* **优化**：在 `read_file` 参数中增加 `offset` 字段，允许 AI 拿到截断提示后发起分片请求。

### **7.5 `MutationObserver` 范围过大与硬编码延时竞态**
* **问题**：全量监听 `document.body` 消耗性能；500ms 硬编码延时在网页卡顿时易导致发送按钮未激活即被点击。
* **优化**：缩小监听范围至主要聊天容器；对发送按钮的 `disabled` 属性进行动态轮询检测，取代硬编码 `setTimeout`。
